...
exports.run = async (client, message, args) => {
  if (
    !message.member.roles.cache.has("778000441672859756") &&
  ...
  if (message.member.roles.highest.position <= uye.roles.highest.position)
    return message.channel
      ...
      .then(x => x.delete({ timeout: 5000 }));
  await uye.roles.add("778000442624966697").catch();
  message.channel
    .send(
      new Discord.MessageEmbed()
        .setAuthor(
        ...
        .setColor(0x00bfff)
        .setDescription(
          `${uye} üyesi, ${message.author} tarafından **${reason}** nedeniyle mutelendi!`
        )
    )
    .catch();
  client.channels.cache
    .get("778000445758504983")
    .send(
      new Discord.MessageEmbed()
        .setAuthor(
          message.member.displayName,
